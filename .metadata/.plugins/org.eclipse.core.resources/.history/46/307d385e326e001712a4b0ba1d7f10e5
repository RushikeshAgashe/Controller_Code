...
void Is_SVPWM(void)
{
...
#if CONTROLLER_ID == DSP1
    EPwm1Regs.CMPA.half.CMPA    = Duty_A;       //PWM1A
    EPwm1Regs.CMPB              = Duty_B;       //PWM1B
    EPwm2Regs.CMPA.half.CMPA    = Duty_C;       //PWM2A
#endif

#if CONTROLLER_ID == DSP2
    EPwm2Regs.CMPB              = Duty_A;       //PWM2B
    EPwm3Regs.CMPA.half.CMPA    = Duty_B;       //PWM3A
    EPwm3Regs.CMPB              = Duty_C;       //PWM3B
#endif

#if CONTROLLER_ID == DSP3
    EPwm4Regs.CMPA.half.CMPA    = Duty_A;       //PWM4A
    EPwm4Regs.CMPB              = Duty_B;       //PWM4B
    EPwm5Regs.CMPA.half.CMPA    = Duty_C;       //PWM5A
#endif

#if CONTROLLER_ID == DSPtest
    EPwm1Regs.CMPA.half.CMPA    = Duty_A;       //PWM1A
    EPwm1Regs.CMPB              = Duty_B;       //PWM1B
    EPwm2Regs.CMPA.half.CMPA    = Duty_C;       //PWM2A
    EPwm2Regs.CMPB              = Duty_A;       //PWM2B
    EPwm3Regs.CMPA.half.CMPA    = Duty_B;       //PWM3A
    EPwm3Regs.CMPB              = Duty_C;       //PWM3B
    EPwm4Regs.CMPA.half.CMPA    = Duty_C;       //PWM4A
    EPwm4Regs.CMPB              = Duty_C;       //PWM4B
    EPwm5Regs.CMPA.half.CMPA    = Duty_C;       //PWM5A
    EPwm5Regs.CMPB              = Duty_C;       //PWM5B
    EPwm6Regs.CMPA.half.CMPA    = Duty_C;       //PWM6A
    EPwm6Regs.CMPB              = Duty_C;       //PWM6B
#endif
}
